...
namespace Dawg.Solver.Winform
{
    public class Solver
    {
        ...
        public IExtendedTile DeserializeTile(string s)
        {
            ...
            if (l.Count() > 6 && l[6] != "") t.Letter = Game.DefaultInstance.Solver.Find(l[6][0]);

            return t;
        }
        public static Letter DeserializeLetter(string s)
        {
            var l = s.Split(';');
        ...
        private HashSet<Word> ExtendRight( string partialWord, ref List<Letter> leftLetters, int minSize, int noeud, int ligne, int colonne)
        {
                        ...
                        for (int l = 0; l < leftLetters.Count; l++)
                        {
                            ...
                            else if (l < leftLetters.Count() && leftLetters[l].Char == Dico.Joker)
                            {
                                ...
                                if (((t.Controlers.ContainsKey(i - 1) && t.Controlers[i - 1] > 0)) || t.Controlers[26] == 26)
                                {
                                    ...
                                    newWords.UnionWith(ExtendRight( partialWord + char.ToLower((char)(i + Dictionnaire.AscShift)), ref leftLetters, minSize, Dico.Legacy[i, noeud], ligne, colonne + 1));
            ...
            else
            {
                ...
                {
                    newWords.UnionWith(ExtendRight( partialWord + t.Letter.Char, ref leftLetters, 1, Dico.Legacy[(int)char.ToUpper(t.Letter.Char) 
